# フェーズ5-Step13 計画書: UI層（ViewModels）のDIサービス接続・残存静的参照の撲滅
...
- **§2.1 フォールバック実装・シム維持の原則**:
  - 各 ViewModel が公開している XAML バインディングプロパティ名・型は一切変更せず、内部の読み書きロジックのみを DI サービス呼び出しに差し替える。
...
- **§3.1 DI (Dependency Injection) の実装（最重要）**:
  - **Pure DI（純粋コンストラクタ注入）の堅持**: `ViewModelFactory` を通じてサービスを明示的に注入し、ViewModel 内部に `AppHost.Services`（Service Locator）を持ち込むアンチパターンを厳禁とする。
- **§3.2 巨大ファイルの編集方針**:
  - 各 ViewModel 内の改修は、静的アクセスを行っているメソッド単位でピンポイントに置換する。
...
`Phase5-Step1-legacy-delegation-audit-report.md` §4-5 ④ に基づき、全 ViewModel 群および `IViewModelFactory`（`ViewModelFactory.cs`）を対象とする。

1. **外枠のみの DI 化（Phase 4 残余課題）**:
   Phase 4 で `IViewModelFactory` が導入され、各 ViewModel のコンストラクタでサービスを受け取る基盤が整った。しかし、ViewModel 内部の業務ロジックには依然として数百箇所の `Global.ProfilePath[...]`、`Global.SaveSettings()`、`Program.rootHub.DS4Controllers` が直参照として残存している。
2. **バックエンドの完成と接続待機**:
   Step 2〜12 により、プロファイル・設定・アクション・デバイス・スロットの全バックエンドが純粋な DI 契約として完成した。本 Step 13 は、これらを UI 層（フロントエンド）に接続し、静的アクセスを一掃する総仕上げステップである。
...
事前検討に基づき、**論点1：案A（マイクロステップによるコア 4 大 ViewModel 優先の順次改修）** および **論点2：案1（Pure DI 原則の堅持: `ViewModelFactory` 経由の明示的コンストラクタ注入）** を採用する。
...
### 1.4 UI スレッド安全性の保証とイベント購読解除
- サービス側から発火される変更イベント（`SettingChanged`、`DeviceAttached` 等）を ViewModel で購読する際は、**必ず `Application.Current.Dispatcher.Invoke / BeginInvoke` を介して UI プロパティを更新**する。
...
| **イベント購読によるメモリリーク** | 中 | `IDisposable` を実装し、画面終了時にイベントハンドラーの解除を徹底する（§1.4）。 |
