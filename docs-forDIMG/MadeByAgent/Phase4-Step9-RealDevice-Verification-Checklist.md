...
| # | 確認内容 | 確認手順 | 結果 | メモ |
|---|---|---|---|---|
| 2-1 | Settings タブで各種環境設定（最小化起動、通知設定、言語設定等）が正常に表示・変更・保存される | Settings タブを開き、各チェックボックスや数値を変更してアプリを再起動し、変更が維持されることを確認 | [○] |ただし、通知部分の設定で、チェックを外してもプロファイル切換時の独自通知が表示され続ける状態になる←DI化後で要調査|
| 2-2 | Log タブでログメッセージがリアルタイムに追記・表示され、クリアボタンやレベルフィルタが動作する | Log タブを開き、コントローラー操作やプロファイル切替時のログが正常に追記されることを確認 | [△] |ログの表記機能自体は問題ないが"Controller 1 is now using Profile ???"が変更前のプロファイルの名前で表示される場合がある←DI化後に要調査|
| 2-3 | About タブでバージョン文字列、アプリタイトル、GitHub リンクが正常に表示・動作する | About タブを開き、バージョン番号（`v...`）が正しく表示され、リンククリックでブラウザが開くことを確認 | [○] |機能自体はOK←ただリンク先や内容が最新のものになっているかDI化後に要調査　|
...
| 3-4 | AutoProfiles（自動プロファイル画面）が Factory 経由で正常に開き、プログラム割り当ての追加・削除ができる | Auto Profiles タブを開き、アプリケーションへのプロファイル割り当ての追加・削除が正常に行えることを確認 | [△] |アプリに応じたプロファイルの切換自体はされるがプロファイル切換時の独自デスクトップ通知が発生しない、Add Steam Gamesが機能していない←DI化後に要調査|
