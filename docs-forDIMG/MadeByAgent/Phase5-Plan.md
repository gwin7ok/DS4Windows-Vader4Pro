...
---
  ...
  * **【Watchpoints 調査報告書の統合対応】**:
    * **[Watchpoint 1 の処置]**: MainWindow 内に残存するプロファイル存在判定（`Global.FindValidProfile` 等の分類 C）を `IProfileRepository` へ委譲切り替え（UI専用定数・テーマは維持）。
    * **[Watchpoint 3 の処置]**: `ProfileListHolder` 等の旧静的ホルダーの実体を DI サービスの透過シムへ一本化（SSOT担保）し、`[Obsolete]` 属性を付与。
    * **[神クラス第一次ダウンサイズ]**: Step 13 で UI 層からの呼出が途絶え、呼出元が 0 件となった `ControlService` / `Global` の不要シムメソッドを物理削除。
