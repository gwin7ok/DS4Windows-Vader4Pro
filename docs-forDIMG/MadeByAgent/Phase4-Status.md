...
| Step 10-2 | 呼び出し元の実稼働DIサービス直接参照化（フェーズ5前倒し・先行着手） | **Stage2-B-3完了・検証待ち、次は B-4** | 2026-09-02 | `ProfileEditor` のプロファイル読込・保存・手動適用を DI 経由へ接続。`ControlService` に `IProfileSettingsService` を注入し、通知・L2/R2 トリガー設定参照を DI 直接化。B-3はユーザー側テスト・コミット待ち。`Mapping.cs` の入力ループ・出力処理は B-4 で扱う。 |
