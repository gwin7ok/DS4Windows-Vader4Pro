...
| Step 10-2-C | Legacy 経路残存の整理と段階移行 | **C-0〜C-4完了、C-5-1着手** | 2026-09-02 | 詳細は `Phase4-Step10-2-C-Plan.md`。C-4の実機ログ確認を完了。C-5-1では`linkedProfileCheck`呼び出し元をDIサービスAPIへ移行する。 |
...
| Step 10-2-C-5 | Legacy シムのログ網羅性監査 | **C-5-1着手（現状参照箇所固定済み）** | 2026-09-02 | 高頻度ログを抑制しつつ、`linkedProfileCheck`の`ControlService`、`ControllerListViewModel`、`ScpUtil`呼び出し元をDIサービスAPIへ移行する。 |
