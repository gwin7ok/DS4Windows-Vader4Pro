...
| Step 10-2-C | Legacy 経路残存の整理と段階移行 | **C-0〜C-5-1完了、C-5-2計画済み** | 2026-09-02 | 詳細は `Phase4-Step10-2-C-Plan.md`。`linkedProfileCheck`の移行と実機確認を完了。次は`tempprofilename`／`useTempProfile`をDIサービスAPIへ移行する。 |
...
| Step 10-2-C-5 | Legacy シムのログ網羅性監査 | **C-5-1完了、C-5-2計画済み** | 2026-09-02 | `linkedProfileCheck`のDI API移行と実機確認を完了。`tempprofilename`／`useTempProfile`の呼び出し元移行計画を作成済み。 |
