...
| Step 10-2 | 呼び出し元の実稼働DIサービス直接参照化（フェーズ5前倒し・先行着手） | **Stage2-B-2進行中（読込・新規保存の検証完了、手動適用の契約整理待ち）** | 2026-09-02 | `ProfileSettingsViewModel` の設定カテゴリ全般を DI 直接参照化。`ProfileEditor` のプロファイル読込・新規保存を `IProfileRepository` 経由へ接続し、Repository の指定名ロードも修正。読込・新規保存はユーザー側テスト・コミット・リモート反映完了。手動 `ApplyProfile` は停止・通知・デバイス反映を含むため `IProfileSwitcher` 契約整理後に移行する。`Mapping.cs` の入力ループ・出力処理は B-4 で扱う。 |
