...
| Step 10-2 | 呼び出し元の実稼働DIサービス直接参照化（フェーズ5前倒し・先行着手） | **Stage2-B-2実装完了・検証待ち** | 2026-09-02 | `ProfileEditor` のプロファイル読込・保存・手動適用を DI 経由へ接続。`IProfileSwitcher.ApplyManualProfile` を追加し、停止・通知・デバイス反映を含む既存引数を維持。`IProfileRepository` の指定名ロードも修正済み。B-2全体はユーザー側テスト・コミット待ち。`Mapping.cs` の入力ループ・出力処理は B-4 で扱う。 |
