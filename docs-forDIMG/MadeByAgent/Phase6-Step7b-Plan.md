...
- `ProfileEditor` のコンストラクタ・`Reload` に `targetDevice` を追加し、`deviceNum` は常に `TEST_PROFILE_INDEX` を使う。`ProfileSettingsViewModel` のコンストラクタに `targetDevice` を追加し、`FuncDevNum` を §2.1 のとおりにする。
...
- 7b-0: 約 1 時間（調査）
- 7b-1: 約 2 時間
- 7b-2: 約 3 時間（校正・入力読み取りの付け替えと確認を含む）
- 7b-3: 約 1.5 時間
- 7b-4: 約 4 時間（テスト、文書、実機確認）
- **合計: 約 1.5〜2 日**（不確実性は 7b-0 の調査結果、特に `BindingWindow` 等のコントローラー直接アクセスの有無による）

## 8. 完了判定チェックリスト
- [ ] Step7b-0 の調査項目 1〜6 が確認され、本書へ追記されている
...
- [ ] ランブルテスト、ライトバー色プレビュー、入力読み取り表示、各種校正が `targetDevice` で動く
- [ ] 一覧経由の編集の動作が従来と変わらない
