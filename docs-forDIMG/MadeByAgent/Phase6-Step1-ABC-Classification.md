...
## タスク2 進捗（2026-09-18）
- 契約差検証完了: ProfileApplicationService（4スロット固定・戻り値非反映）、SpecialAction（GetAction/LoadActions正規化差、Save/Remove操作境界差）、ConfigurationLocation（保存場所とXMLパス境界）、StandardActionInit（複数XML更新・再読込）
- 分類確定進捗: a（既存契約候補）= ControlService設定getter、Mapping設定、Mouse入力経路 / b（要設計）= 上記契約差項目 + IKbmBackendLifecycle、IDisplayCoordinateService / c（除外・保留）= const、Pre-Host、テスト参照、フォールバック互換、readonly配列/辞書（自動除外しない）
- 未完了: 762参照の個別一意ID（ファイル+行+列）による完全台帳生成、各参照の厳密a/b/c確定、ホットパス判定、分類別実測集計生成
