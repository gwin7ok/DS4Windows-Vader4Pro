...
| Step2-PR-5 | 実機確認の全項目（実施した場合は不要）: 通常操作、タッチパッドのトグル、KBM 出力、スペシャルアクション・エクストラの動作、バッテリー表示のトレイアイコン、遅延警告時の点滅、DS4 トリガーモード、OSC／UDP。あわせて On_Report の処理時間（アプリの入力遅延・出力遅延の最大値表示）を PR-4 の状態と比較 | セクション2、3、7、8、9 | レビュー時に判断 |
