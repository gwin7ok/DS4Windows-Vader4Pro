...
- [x] 実機シナリオ2相当（L2を押しながらPSを長押し）: 実機ログ（`ds4windows_log.txt`、20:33:41.6589-42.9503付近）の解析により、プロファイル適用直後にトリガーが押されっぱなしの間（約1.3秒間）は`Trigger detected`のみ記録され続け`SpecialAction PROFILE: Triggered`（実行）は発生しないこと、実際に一度離して押し直した瞬間（約0.39秒の検知空白の直後）に初めて1回だけ実行されることを確認した。仕様④（`RequiresFreshPressAfterReset`）が意図通り機能していることをログで確認済み。
- [x] 実行重複の有無: `ApplyProfile CALLED`/`COMPLETED`の全件をスクリプトで突き合わせ、ログ全体を通じて一度もオーバーラップ（前の処理完了前に次のCALLEDが発生する状態）が発生していないことを機械的に確認した（`IsExecuting`ガードの正常動作を裏付け）。
- [x] 正常系（都度離して押し直す短いタップの繰り返し、計12回超）: 押すたびに1回ずつ正しく切り替わっており、二重発火・見落としのいずれも発生していないことを確認した。
- [ ] gwin7ok氏より、PSダブルタップ（実機シナリオ1相当・ケース1）についても正常動作の報告あり。ログ上の該当箇所の詳細な行番号特定は今回未実施（既存の正常系確認で代替済みと判断）。
- **備考**: ログ後半に別途確認された`0101_GI_マップ`等のGuide複合キー連射は、既に切り出し済みの別課題「Issue 8-3」（`Phase5-Step14-RealDevice-Investigation-and-Fix-Report.md` §7.4）に該当するものであり、本タスクの対象外・想定通りの残存事象である。
...
1. [x] `ActionInstanceState`に`RequiresFreshPressAfterReset`・`IsExecuting`・`PendingReExecutionRequested`が追加されていること。
2. [x] `Profile`型（および該当する`Program`/`GyroCalibrate`型）のトリガー実行判定が、上記フィールドを用いたガードを経由するよう修正されていること。
3. [x] §2タスク4の単体テストが全て追加され、PASSすること。
4. [x] 実機シナリオ（§2タスク5）で、報告されていた暴走ループおよび実行オーバーラップが再現しないことを確認すること。
5. [x] 既存の全自動テストが回帰なくPASSすること。

**【2026-09-12】上記1〜5全て達成。Issue 8-1本体（④是正・実行重複禁止）は完了とする。**
