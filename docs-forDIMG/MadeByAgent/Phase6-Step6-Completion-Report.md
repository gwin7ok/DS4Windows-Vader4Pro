...
作成日: 2026-09-24  
状態: **実装完了**（Step6-1・6-2 はビルド・テスト成功。Step6-3 の追加テストと実機確認は確認待ち。確認結果を §6 に追記して完了確定とする）  
...
## 6. 検証結果
- **ビルド・テスト（ユーザー確認、2026-09-24）**: Step6-1、Step6-2 の時点で、ビルド・テストビルド・テスト実行がすべて成功。Step6-3 の追加テストは確認待ち。
- **実機確認**（`Phase6-Step6-Plan.md` §5）: 未実施（確認待ち）。
...
- [ ] 編集画面を開いたとき・プリセット適用後のボタン名が出力種別と一致する（実機確認待ち）
- [x] `PluginSlot`／`UnplugSlot` の TODO を案R の内容に書き換え、接続作業を Step10 §8 に登録
- [x] ホットスワップ機構に手を加えていない
- [ ] Step6-3 の追加テストを含むビルド・テスト全件成功（確認待ち）
- [ ] 実機確認が完了、または Step11 の先送り台帳に登録
