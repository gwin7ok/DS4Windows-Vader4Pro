...
現在ステータス: **実機検証中（Issue 7 是正フェーズC 着手 / 次回: タスク(c)-1）**
...
| タスクID | タスク概要 | 対象ファイル | 状態 | 備考 |
|---|---|---|:---:|---|
| **(c)-1** | `ProfileSettingsViewModel.cs` ロード時 `tempConType` 再同期 | `ProfileSettingsViewModel.cs` | ⏳ **次回着手** | `UpdateProperties` 時の `tempConType` 再代入 ＆ PropertyChanged 発火 |
| **(c)-2** | `ControllerTypeIndex` / `EnableOutputDataToDS4` 双方向連動ガード | `ProfileSettingsViewModel.cs` | ⏳ 未着手 | 矛盾検知時のフェイルセーフ連動ガード |
| **(c)-3** | 全単体テスト・クリーンビルド確認 | ソリューション全体 | ⏳ 未着手 | 全テスト PASS 維持およびビルド確認 |
...
| タスクID | タスク概要 | 対象ファイル | 状態 | 備考 |
|---|---|---|:---:|---|
| **(d)-1** | ドキュメント・進捗ステータス更新（旧タスク-4） | 計画書・報告書・Status文書 | ⏳ 未着手 | Issue 6 ＆ Issue 7 是正完了を記録 |
| **実機CP4** | 実機起動・終了時の設定永続化検証 | 実機環境 | ⏳ 未着手 | ウィンドウ幾何情報・カラム幅・通知・Emulated Controller の XML 保存・復元確認 |
