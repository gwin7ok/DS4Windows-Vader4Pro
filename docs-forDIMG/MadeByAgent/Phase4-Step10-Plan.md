# フェーズ4-Step10 計画書: Phase3引継ぎ再確認・シム整理・[DI]ログ整備 & 最終実機検証 CP4
...
- **§2.3 ログ出力の厳格な維持**:
  - `AppLogger.LogToGui` 等の既存ログ出力を厳格に維持しつつ、DI 実行経路に Trace レベルの `[DI]` プレフィックスログを付与する。
...
- **Step10 で行うこと**:
  - **1. [DI] Trace ログの整備**: DI 化後の新経路を通って処理が行われる際、ログに `[DI]` を含む Trace レベルの詳細ログが出力されるように各サービス・エントリポイントを整備し、DI 経路の稼働状況を可視化する。
...
### 1.1 [DI] Trace ログ出力の設計方針
各 DI サービスおよび Composition Root の主要エントリポイントにおいて、Trace レベルで `[DI]` プレフィックスを付与したログを出力する。
...
// 2. ProfileSettingsService / ProfileRepository 実行時
AppLogger.LogToGui($"[DI] ProfileSettingsService.SetTouchpadActive: Slot {deviceIndex} = {value}", false, true);
AppLogger.LogToGui($"[DI] ProfileRepository.LoadProfile: Slot {deviceIndex}, Profile '{profileName}' loaded via DI", false, true);

// 3. ViewModelFactory 実行時
...
| `DS4Windows/DS4Control/Services/ProfileSettingsService.cs` | 更新 | **DI永続資産** | `[DI]` Trace ログ出力の追加 |
| `DS4Windows/DS4Control/Services/ProfileRepository.cs` | 更新 | **DI永続資産** | `[DI]` Trace ログ出力の追加 |
| `DS4Windows/DS4Control/Services/SpecialActionRepository.cs` | 更新 | **DI永続資産** | `[DI]` Trace ログ出力の追加 |
| `DS4Windows/DS4Control/Services/DeviceStateService.cs` | 更新 | **DI永続資産** | `[DI]` Trace ログ出力の追加 |
| `DS4Windows/DS4Control/Services/OutputSlotService.cs` | 更新 | **DI永続資産** | `[DI]` Trace ログ出力の追加 |
| `DS4Windows/DS4Control/Services/ViewModelFactory.cs` | 更新 | **DI永続資産** | `[DI]` Trace ログ出力の追加 |
| `docs-forDIMG/MadeByAgent/Phase4-Step10-RealDevice-Verification-Checklist.md` | 新規 | ドキュメント | 最終実機動作確認チェックリスト CP4 |
| `docs-forDIMG/MadeByAgent/Phase4-Step10-Plan.md` | 新規 | ドキュメント | 本計画書 |
| `docs-forDIMG/MadeByAgent/Phase4-Step10-Completion-Report.md` | 新規 | ドキュメント | Step10完了報告書（Phase 4 全体完了報告書） |
| `docs-forDIMG/MadeByAgent/Phase4-Status.md` | 更新 | ドキュメント | 進捗ステータス更新（Phase 4 全体完了） |

---

## 3. 作業手順（マイクロタスク分割）

### タスク Step10-1: DI 実行経路への [DI] Trace ログ出力の整備
- 各 DI サービス、`ViewModelFactory`、および `AppHost` に `[DI]` プレフィックス付きの Trace ログ出力を追加。
