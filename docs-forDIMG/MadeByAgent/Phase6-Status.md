...
最終更新日: 2026-09-20  
状態: Phase6-Step1 完了 / Step2 実施中（PR-1〜PR-3 完了・PR-4 実装済みでレビュー待ち）/ Step13-1（配置整理 20件）完了 / Step13（.cs ファイル配置の統一）・Step10b（1ファイル1型の全数是正）新設 / Step3〜Step12 計画確定・承認待ち  
...
| **Step 2** | `ControlService.cs` 解体 | `ControlService.cs` | 66箇所 | **実施中（56/66 ID 実装済み）** | `Phase6-Step2-Plan.md` | PR-1〜3 完了 (2026-09-19)、PR-4 実装済み（レビュー待ち）、PR-5〜6 残 |
...
- **進捗率**: **約85%（66 ID 中 56 ID 実装済み。PR-4 はレビュー待ち。C2-02 は決定D2 により防御コードとして温存）**
...
- **実装済み（レビュー待ち）**: PR-4（ID: C2-51〜54, 56, 58, 64, 65。`IAppSettingsService.FlashWhenLate`、`IDeviceStateService.OnDeviceSerialChange` を追加。修飾 `Global.` 行は 14 → 11 行）。
- **残り**: PR-5（毎レポート最頻度経路、ID: C2-50, 55, 57, 59〜63, 66）→ PR-6（`using static` 削除・最終確認）。
- **計画ハイライト**:
...
1. **Step2-PR-4** のビルド・テスト・実機確認を行い、コミットする。
2. **Step2-PR-5**（毎レポート最頻度経路、ID: C2-50, 55, 57, 59〜63, 66）に着手する。
