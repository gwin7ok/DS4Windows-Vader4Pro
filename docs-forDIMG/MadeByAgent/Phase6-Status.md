...
| **Step 7b** | プロファイル編集の即時反映廃止（保存・適用時に一括反映） | `ProfileEditor.xaml.cs`, `MainWindow.xaml.cs`, `ProfileSettingsViewModel.cs` | 編集スロット固定＋`targetDevice`導入 | 決定1〜7 確定（すべて推奨案）、Step7b-1 完了、Step7b-2 実装済み（2026-09-26、ビルド・テスト確認待ち） | `Phase6-Step7b-Plan.md` | 実装未着手（Step7 の後・Step8 の前） |
...
- **計画ハイライト**:
  - 孤立配列と、常に `None` を返す API を削除し、二重管理の根本原因を抹消。`OutputSlotChanged` イベントは維持。
  - 「永続設定（`OutContType`）」「実行時接続（`ActiveOutDevType`）」「UI一時状態（`OutDevTypeTemp`）」の三態台帳を確立。
  - 案Y（所有権の移動）は Phase7 へ引き継ぎ（`Phase6-Step12-Plan.md` §4、`DI-App-Wide-Migration-Plan.md` §6.9.3 に登録済み）。Phase8 には該当なし。

---

### Phase6-Step6: ProfileEditor のマッピング一覧の機種表示追従 ＆ 未接続API整理【完了】
- **進捗率**: **100%（Step6-0〜6-3、2026-09-24 完了確定）。ビルド・テストビルド・テスト実行成功、コミット・リモート反映済み。実機確認 1〜4 問題なし。完了報告書: `Phase6-Step6-Completion-Report.md`**
- **副次的な観察**: 仮想コントローラー種別変更時の Windows トースト通知が表示されない件は、アプリの回帰ではなく Windows 側の要因と判断（旧ブランチでも再現）。別途調査として `Phase6-Step11-Plan.md` §3.3 に登録。
- **規約追加**: 未参照コードの扱いの判断手順を `.github/copilot-instructions.md` §2.4 に明文化（2026-09-24）。
- **確定方針（2026-09-24 改訂）**:
...
- **Step7-4 実装内容（2026-09-25 実装。実機確認で効果なしと判明：ホストが `Global` の静的初期化［`ScpUtil.cs:779`］で起動引数なしに先に作られ、298 行の `CreateHost(config, parser)` が空振りしているため。2026-09-25 に決定7＝案H［起動引数の保持役 `IStartupArguments`（新設）を DI に登録し、`AppHost.CreateHost(config, parser)` が既存ホストに対しても値を設定］で再実装。ビルド・テストビルド・テスト実行成功、実機で `-virtualkbm fakerinput` の DEBUG ログが `for fakerinput` になることを確認。根本原因（案R）は持ち越し K7-1、FakerInput 未導入時に `fakerinput` 指定でフォールバックしない既存の動作は K7-2［§6.5］。モデル図 03・04 に追記）**: 決定5＝A。`ServiceRegistration.cs` の `ControlService` の登録で、空の `new ArgumentParser()` の代わりに DI コンテナに登録された実際の起動引数（`sp.GetService<ArgumentParser>() ?? new ArgumentParser()`）を渡すよう修正し、2026-09-02 以降無視されていた起動引数 `-virtualkbm` を復旧。テスト `ServiceRegistrationArgumentParserTests.cs`（2件、`ControlService` を実際に生成して `cmdParser` を確認）を新設。
