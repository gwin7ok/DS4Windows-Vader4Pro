...
最終更新日: 2026-09-20  
状態: Phase6-Step1 完了 / Step2 実施中（PR-1〜PR-4 完了・PR-5 実装済みでレビュー待ち・残りは PR-6）/ Step13-1（配置整理 20件）完了 / Step13（.cs ファイル配置の統一）・Step10b（1ファイル1型の全数是正）新設 / Step3〜Step12 計画確定・承認待ち  
...
| **Step 2** | `ControlService.cs` 解体 | `ControlService.cs` | 66箇所 | **実施中（65/66 ID 実装済み）** | `Phase6-Step2-Plan.md` | PR-1〜4 完了、PR-5 実装済み（レビュー待ち）、PR-6 残 |
...
- **進捗率**: **約98%（66 ID 中 65 ID 実装済み。PR-5 はレビュー待ち。C2-02 は決定D2 により防御コードとして温存）**
...
- **実装済み（レビュー待ち）**: PR-5（ID: C2-50, 55, 57, 59〜63, 66。入力ループ最頻度経路。`Global.` 修飾行は 11 → 8 行。On_Report 相当の取得16個がヒープ割り当て0であることをテストで固定）。
...
1. **Step2-PR-5** のビルド・テスト・実機確認（または Step11 への先送り）を行い、コミットする。
