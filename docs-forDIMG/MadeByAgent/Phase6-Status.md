...
最終更新日: 2026-09-20  
状態: Phase6-Step1 完了 / Step2 実施中（PR-1〜PR-4 完了・PR-5 反映済み・テスト1件の誤りを修正中・残りは PR-6）/ Step13-1（配置整理 20件）完了 / Step13（.cs ファイル配置の統一）・Step10b（1ファイル1型の全数是正）新設 / Step3〜Step12 計画確定・承認待ち  
...
| **Step 2** | `ControlService.cs` 解体 | `ControlService.cs` | 66箇所 | **実施中（65/66 ID 実装済み）** | `Phase6-Step2-Plan.md` | PR-1〜4 完了、PR-5 反映済み（テスト修正の再確認待ち）、PR-6 残 |
...
- **進捗率**: **約98%（66 ID 中 65 ID 実装済み。PR-5 はテスト1件の修正の再確認待ち。C2-02 は決定D2 により防御コードとして温存）**
...
- **反映済み（テスト1件の修正の再確認待ち）**: PR-5（ID: C2-50, 55, 57, 59〜63, 66。ビルド・テストビルド成功、テスト実行で1件失敗〔テストの誤り、修正版を提供〕。入力ループ最頻度経路。`Global.` 修飾行は 11 → 8 行。On_Report 相当の取得16個がヒープ割り当て0であることをテストで固定）。
- **残り**: PR-6（`using static` 削除・最終確認。事前調査で、外すと未解決になる名前は `vigemInstalled` の1件のみと判明）。
- **計画ハイライト**:
...
1. **Step2-PR-5** の修正版テストで全件合格を確認し、コミットする。実機確認は実施するか Step11 へ先送りする。
2. **Step2-PR-6**（`using static` 削除・最終確認）に着手する。その後、Step13-2〜13-5（配置整理）を実施し、Step3 に進む。
