...
最終更新日: 2026-09-20  
状態: Phase6-Step1 完了 / Step2 実装完了（PR-1〜PR-5 完了、PR-6 は反映済みでテスト1件が実行順序依存で失敗・修正版を提供）/ Step13-1（配置整理 20件）完了 / Step13（.cs ファイル配置の統一）・Step10b（1ファイル1型の全数是正）新設 / Step3〜Step12 計画確定・承認待ち  
...
| **Step 2** | `ControlService.cs` 解体 | `ControlService.cs` | 66箇所 | **実装完了（66/66 ID。PR-6 はテスト修正の再確認待ち）** | `Phase6-Step2-Plan.md` | PR-1〜5 完了、PR-6 反映済み（テスト修正の再確認待ち）。完了確認は計画書 付録C |
...
| **Step 13**| `.cs` ファイル配置の統一 | `DS4Windows/` 配下の44ファイル | git mv のみ | **13-1 完了（20/44）** | `Phase6-Step13-Plan.md` | 13-2〜13-5: Step2 完了後・Step3 着手前 |
| **合計** | **Phase6 全域** | **約40ファイル ＋ 配置統一44ファイル ＋ 分割55ファイル** | **約404箇所** | **計画策定100% / 実装: Step1 完了・Step2 実装完了（PR-6 確認待ち）** | **全Step計画書完備（Step13 を含む）** | - |
...
- **進捗率**: **100%（66 ID 中 66 ID 実装済み。C2-02 は決定D2 により防御コードとして温存。PR-6 はテスト修正の再確認待ち）**
...
- **反映済み（テスト1件の修正の再確認待ち）**: PR-6（ビルド・テストビルド成功、テスト実行で1件失敗〔テストの実行順序依存、修正版を提供〕。`using static DS4Windows.Global;` の削除と `Global.vigemInstalled` への修飾。差分2行。`ControlService.cs` の `Global.` 参照は9箇所、すべて除外項目と C2-02。回帰ガードテスト `ControlServiceGlobalReferenceGuardTests` を追加）。
- **残り**: PR-6 の修正版テストの確認とコミットのみ。その後、Step2 の完了確認（計画書 付録C）を締結する。
- **計画ハイライト**:
...
### Phase6-Step13: `.cs` ファイル配置の統一【実施中】
- **進捗率**: **13-1 完了（20/44 ファイル、約45%）。13-2〜13-5 は Step2 の完了後に実施**
...
1. **Step2-PR-6** の修正版テストで全件合格を確認し（ビルドの警告数も確認）、コミットする。
2. Step13-2〜13-5（配置整理）を実施する。
3. **Step3**（`Mapping.cs` の Global 参照解消）に着手する。
