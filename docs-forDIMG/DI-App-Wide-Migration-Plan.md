...
### 6.13 フェーズ8（新設、2026-09-23）: Controls／SpecialActions統一トリガーディスパッチ

#### 6.13.1 発端と位置づけ

ユーザー提案（2026-09-23）: 「Controls タブと SpecialActions タブは設定場所が異なるだけで、
『トリガーが成立したら処理を実行する』という点では同じ。UI と設定の保存/読込は分離を維持したまま、
プロファイル読込時にトリガーキーと処理の組み合わせをオブジェクト化し、コントローラー接続時にそれを
紐づけ、周期的にトリガーを監視する、という単一の実行時経路に統合できないか」という提案を受け、
実現可能性を調査した。

#### 6.13.2 調査で判明した事実

1. **SpecialActions側には既に2系統の抽象化が存在する**。`IActionBinding`／`IInputAction`／
   `IOutputAction`／`IActionRegistry`（系統A）はご提案に近い形をしているが、**実際の呼び出し元が
   ゼロで未配線**（`IActionRegistry.Register`/`GetBindingsForDevice`は呼ばれておらず、
   `KeyActionBinding.Input`は`=> null;`のまま）。実際に稼働しているのは`Actions.Action`／
   `IActionFactory`／各種Adapter（系統B、Phase1で構築・配線済み）。
2. **「トリガー成立判定」自体は、系統A・Bのどちらにも属さず、`Mapping.cs`の`MapCustomAction`に
   手続き的に埋め込まれている**（デバウンス、抑制ロジック`CheckForSpecialActionSuppression`、
   トグル処理等を含む）。統合の中心課題はここになる。
3. **Controls タブ側（`DS4ControlSettings`）には対応する抽象化が一切ない**（単純な1ボタン1設定）。
...
フェーズ7（`Mapping.cs`完全instance化）の完了後に着手する。理由はフェーズ6→7の関係と同じで、
static結合を先に減らしておくことが本フェーズの下地になり、また複数の大きな関心事
（instance化そのものと、ディスパッチ経路の統合）を同時に扱うリスクを避けるため。

#### 6.13.5 モデル図への影響

本設計は `02-Layer-Architecture-Diagram.md` の2-a／2-b層の区分と、`01`・`03`・`04`の対応するすべてに
影響する。具体的な変更案は決定事項D1〜D5が確定してから提示し、**実際の変更はユーザーの指示を
得てから行う**。
...
6. フェーズ7完了後、`Phase8-Unified-Trigger-Dispatch-Plan.md`の決定事項D1〜D5を確定し、フェーズ8
   （Controls／SpecialActions統一トリガーディスパッチ）に着手する。
