...
実装ルール（必須）
- アクションインスタンスのライフサイクル:
  - 各 `Action`（例: `KeyAction`, `ButtonAction`, `MacroAction`）のインスタンスは、プロファイルが適用された後に初めてそのアクションが実行される時点で生成される（遅延生成）。
  - その後は同一プロファイル適用期間中、同じインスタンスを再利用する（状態はインスタンスのプロパティ／`ActionInstanceState` が保持する）。
  - 新しいプロファイルが適用されるか、コントローラが切断された場合は当該プロファイルに紐づく全アクションインスタンスを破棄し、次回実行時に再生成する。

- リピータと合成キー送出の責任分担:
  - 連続キー送出（リピート）は必ずコントローラ層（例: `KeyButtonActionController`）が `RepeatHelper` を用いて管理する。
